...
**Qué SÍ hace:** cotizar con precio exacto, responder FAQs, avisar al
equipo cuando alguien quiere reservar, y reconocer el mensaje pre-armado que
manda el botón "Consultar disponibilidad" del sitio (`script.js` →
`buildWhatsAppLink`) — si ya trae fechas y habitación válidas, cotiza de
inmediato sin pedirle al huésped que repita lo que ya eligió en la página
(ver `parseWebsiteQuoteMessage` en `conversationEngine.js`).
...
## Disponibilidad, facturación y alertas al staff

- **Fechas sin disponibilidad:** se marcan en `blackout_dates` dentro de
  `hotel-data.json` (fecha `start`/`end` inclusivas, y opcionalmente
  `room_type_ids` si el bloqueo es solo de ciertas habitaciones — vacío o
  ausente = todas). El bot deja de cotizar esas fechas y le pide al cliente
  hablar con el equipo. Esto es manual: alguien tiene que actualizar el JSON
  cuando cambie la disponibilidad real. Llevar esto automático (con
  inventario real por habitación) es justo lo que resuelve el sistema
  centralizado de fase 2.
- **Facturación:** se anuncia en `hotel_info.policies.invoicing` /
  `invoicing_note` de `hotel-data.json`, y el bot ya la menciona si alguien
  pregunta.
- **Cómo se entera recepción de que alguien quiere reservar:** el bot le
  manda un WhatsApp directo al número que configures en
  `HOTEL_STAFF_WHATSAPP_NUMBER` (ver `.env.example`) cada vez que un
  huésped pide un agente, confirma reservación, o pregunta por fechas sin
  disponibilidad. Ojo: WhatsApp solo entrega texto libre a números que le
  hayan escrito al bot en las últimas 24h, así que el staff debe mandarle
  un "hola" al número del bot una vez por turno para mantener esa ventana
  abierta. Una plantilla aprobada por Meta eliminaría esa dependencia —
  pendiente para cuando haya tiempo de someterla a revisión.
- **Grupos grandes (más de 6 personas, o el tope de la habitación más
  grande que tengan cargada):** el bot no arma combinaciones de varias
  habitaciones automático — no lleva inventario/disponibilidad real por
  habitación. Cuando detecta un grupo así, avisa al staff y le pide al
  cliente hablar directo con el equipo. Este es otro punto natural para
  resolver en fase 2.
